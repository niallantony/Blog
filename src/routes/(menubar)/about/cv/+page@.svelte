<script>
  import Icon from "@iconify/svelte";
</script>

<h1>Niall Craven</h1>
<div class="header">
  <h2 class="title">Software Developer</h2>
  <ul class="contacts">
    <li><Icon icon="ri:phone-fill" />+83-10-7686-2858</li>
    <li><Icon icon="ri:mail-fill" />craven.niall@googlemail.com</li>
  </ul>
</div>
<div class="sections">
  <div class="column left">
    <div class="big-section education">
      <h2>Education</h2>
      <div class="school">
        <h3>MSc Computer Science</h3>
        <div class="school-info">
          <p class="school-name">
            <Icon icon="ri:map-pin-2-line" />University of Bath
          </p>
          <p class="school-time">
            <Icon icon="ri:calendar-2-line" />2024 - 2026 (Expected)
          </p>
        </div>
        <ul>
          <li>
            Developed an Object Oriented ATM system in a distributed team, using
            Scrum practices
          </li>
          <li>Created a spam filter using neural networks in Python</li>
          <li>
            Distinction grades in all units with particularly strong coursework
            in Functional Programming, Databases and AI
          </li>
        </ul>
      </div>
      <div class="school">
        <h3>BA Fine Art</h3>
        <div class="school-info">
          <p class="school-name">
            <Icon icon="ri:map-pin-2-line" />Newcastle University
          </p>
          <p class="school-time">
            <Icon icon="ri:calendar-2-line" />2010 - 2014
          </p>
        </div>
        <ul>
          <li>
            Worked with Blender and Adobe Photoshop to create new media art
          </li>
          <li>
            Worked with Baltic Museum, Gateshead, in a digital archiving
            project, including methods for handling large image and video file
            formats in storage.
          </li>
        </ul>
      </div>
    </div>
    <div class="big-section employment">
      <h2>Work Experience</h2>
      <div class="job">
        <h3>Lecturer - Department of Electronics</h3>
        <div class="job-info">
          <div class="job-loc">
            <Icon icon="ri:map-pin-2-line" />Bucheon University - Bucheon, South
            Korea
          </div>
          <div class="job-time">
            <Icon icon="ri:calendar-2-line" />Mar 2021 - Present
          </div>
        </div>
        <div class="job-list">
          <ul>
            <li>
              Lead a number of classes including 'Data Skills for Engineers',
              'English for Engineers' and others
            </li>
            <li>
              Maintained online learning environments for students to use
              distance learning resources
            </li>
            <li>
              Taught students to effectively use databases, SQL queries, and
              data pipelines in Python and Bash
            </li>
            <li>
              Ensured student wellbeing and provided individualised monitoring
              for students' progress
            </li>
          </ul>
        </div>
      </div>
      <div class="job">
        <h3>Online Lecturer - English Language</h3>
        <div class="job-info">
          <div class="job-loc">
            <Icon icon="ri:map-pin-2-line" />Korea Cyber University - Seoul,
            South Korea
          </div>
          <div class="job-time">
            <Icon icon="ri:calendar-2-line" />Sep 2019 - Present
          </div>
        </div>
        <div class="job-list">
          <ul>
            <li>
              Produced written, audio and video resources to teach effective
              English communication
            </li>
            <li>
              Managed students online grades and delivered personalised feedback
              and learning aims
            </li>
            <li>
              Liased with professors on strategies for improving online lecture
              materials
            </li>
          </ul>
        </div>
      </div>
    </div>
  </div>
  <div class="column right">
    <div class="big-section intro">
      <h2>Intro</h2>
      <p>
        Motivated developer with extensive experience in visual design and
        online education, looking to transition into backend development.
      </p>
      <p>
        Proficient in Python, Javascript and currently studying Java (Spring).
        Wealth of experience developing web applications with Postgres, Express,
        React and Node.js.
      </p>
    </div>
    <div class="big-section skills">
      <h2>Skills</h2>
      <ul>
        <li>
          <strong>Programming Languages</strong><span>Javascript, Python</span>
        </li>
        <li>
          <strong>Back-end Development</strong><span
            >REST APIs, node.js, Express.js, SvelteKit</span
          >
        </li>
        <li>
          <strong>Database Management</strong><span
            >PostgreSQL, MySQL, MongoDB, Prisma</span
          >
        </li>
        <li>
          <strong>Front-end Development</strong><span
            >React, Svelte, HTML, CSS</span
          >
        </li>
        <li>
          <strong>Tools & Environment</strong><span
            >Bash, Git, TDD, Agile methodologies, Object-Oriented programming</span
          >
        </li>
        <li><strong>Languages</strong><span>English, Korean</span></li>
      </ul>
    </div>
    <div class="big-section core">
      <h2>Profile</h2>
      <ul>
        <li>
          <Icon
            style="width:20px; height:20px;"
            icon="icon-park:communication"
          />
          <strong>Communication:</strong>
          <span
            >10 years experience in education and visual design has given me the
            skills to clearly and efficiently communicate and convey complex
            ideas, abstract concepts and customers' visions.</span
          >
        </li>
        <li>
          <Icon style="width:20px; height:20px;" icon="icon-park:telescope" />
          <strong>Curiosity:</strong><span>
            The ability to adapt and thrive in various different domains, and to
            continuously improve my skills comes from a deep desire to learn new
            skills and hear new ideas.</span
          >
        </li>
        <li>
          <Icon style="width:20px; height:20px;" icon="icon-park:brain" />
          <strong>Creativity:</strong><span>
            A unique perspective on problems and ability to find creative
            solutions is something I bring into software design and development.</span
          >
        </li>
        <li>
          <Icon style="width:20px; height:20px;" icon="icon-park:compass" />
          <strong>Craftsmanship:</strong><span>
            A love of programming that goes back to childhood: while I get
            excited by new fields and opportunities, I prioritise strong
            fundamental knowledge of a core systems and technologies.</span
          >
        </li>
      </ul>
    </div>
  </div>
</div>

<style>
  :root {
    font-family: Arial, Helvetica, sans-serif;
    --accent: #273469;
    --light-text: #6c757d;
    --light-back: #e0e1dd;
  }
  h1 {
    font-size: 34pt;
    border-bottom: solid 1px var(--light-back);
    margin: 0;
    padding: 20px 20px 0;
  }
  .title {
    margin-top: 6px;
    font-size: 14pt;
    font-weight: 400;
    padding-left: 20px;
  }
  .header {
    display: flex;
    justify-content: space-between;
  }

  .contacts {
    margin-top: 0;
    padding-right: 16px;
    list-style: none;
    display: flex;
  }
  .contacts > li {
    margin: 8px;
    display: flex;
    align-items: center;
    gap: 8px;
  }
  .sections {
    display: grid;
    grid-template-columns: 3fr 2fr;
    background-color: var(--accent);
  }
  .big-section {
    padding-left: 13px;
  }
  .big-section h2 {
    border-bottom: solid 2px var(--accent);
    margin-bottom: 6px;
  }
  .skills > ul {
    list-style: none;
    padding-left: 10px;
  }
  .skills li {
    display: grid;
    grid-template-columns: 120px 1fr;
    border-bottom: solid 1px var(--accent);
  }
  .skills strong {
    padding: 4px 0 4px 6px;
    box-sizing: border-box;
    margin-right: 16px;
    width: 100%;
  }
  .skills span {
    padding: 6px;
  }
  .job-info,
  .school-info {
    margin-right: 16px;
    display: flex;
    justify-content: space-between;
    gap: 16px;
  }
  .job-loc,
  .job-time,
  .school-name,
  .school-time {
    display: flex;
    align-items: center;
    gap: 8px;
  }
  strong,
  h3 {
    color: var(--accent);
  }
  p,
  li,
  .job-loc,
  .job-time {
    font-size: 10pt;
    color: var(--light-text);
  }
  .employment li,
  .education li {
    margin-bottom: 10px;
  }
  .education p {
    margin: 0;
  }

  .intro p {
    padding-left: 16px;
  }
  .right {
    background-color: white;
    padding-right: 20px;
    border-left: solid 1px var(--accent);
    border-radius: 5px 0 0 0;
  }
  .left {
    background-color: white;
    border-radius: 0 5px 0 0;
    border-right: solid 1px var(--accent);
  }
  .header,
  h1,
  .contacts li {
    color: white;
    background-color: var(--accent);
  }
  .left li {
    padding-right: 16px;
  }
  .left h2 {
    position: relative;
  }
  .right .big-section {
    padding-left: 0;
  }
  .right h2 {
    padding-left: 16px;
    position: relative;
    text-align: right;
  }
  .left h2:after {
    content: "•";
    font-size: 34pt;
    position: absolute;
    right: -9px;
    bottom: -28px;
    color: var(--accent);
  }
  .right h2:after {
    content: "•";
    font-size: 34pt;
    position: absolute;
    left: -9px;
    bottom: -28px;
    color: var(--accent);
  }
  .core ul {
    padding-left: 16px;
  }
  .core li {
    display: grid;
    grid-template-columns: 32px 1fr;
    grid-template-rows: 16px;
    margin-bottom: 10px;
  }
  .core span {
    grid-column: 2 / 3;
  }
</style>
