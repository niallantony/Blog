export default {
  en: {
    "headers.about": "Who I am",
    "headers.working": "What I am Working On Now",
    "headers.stack": "What I Use:",
    "headers.studying": "What I Study:",
    "headers.interests": "What Interests Me:",
    "headers.belief": "What I Believe",
    "headers.hobbies": "What I Do Outside the Computer",
    "headers.contact": "How to Contact Me:",
    "headers.projects": "What I Have Worked On Recently",

    "body.about.0":
      "I am someone who grew up around computers and have always been interested in what can be achieved with them. My interest in programming started with designing and writing computer games as a kid, and progressed through my career as an artist and educator. Programming and making things is something I have a passion for, and I hope to be the kind of developer who can blend engineering with creativity.",
    "body.about.1":
      "While I value new technologies and trends in development, I place importance on core skills that I believe will serve me well in any role: programming fundamentals, good system design and effective communication skills.",

    "body.working.0": "building up my personal site",
    "body.working.1": "studying as part of an MSc in Computer Science",
    "body.working.2": "teaching Data Skills to University Students",
    "body.working.3":
      "developing applications in conjunction with accessibility researchers",

    "body.studying.0": "Cloud Engineering",
    "body.studying.1": "Software Fundamentals",
    "body.studying.2": "Backend Development",

    "body.interests.0": "Machine Learning & AI",
    "body.interests.1": "Databases",
    "body.interests.2": "Computer Systems",
    "body.interests.3": "Creative Coding",
    "body.interests.4": "Graphics and Visualisation",

    "body.belief":
      "Technology should be something that serves people, no matter their background or capabilities. I would like to help create software which makes the world a more inclusive place. I believe that software developers have a duty to make reliable and ethical software.",

    "body.hobbies.text":
      "When I am not relaxing with my beautiful wife and two oddball cats, I like to occupy my time with one of many hobbies.",
    "body.hobbies.list.0": "Reading",
    "body.hobbies.list.1": "Drawing & Painting",
    "body.hobbies.list.2": "DIY",
    "body.hobbies.list.3": "Games",
    "body.hobbies.list.4": "Weightlifting",

    "body.contact": "Download my CV (pdf)",
  },

  kr: {
    "headers.about": "소개",
    "headers.working": "현재 하고 있는 일",
    "headers.stack": "사용하는 스택:",
    "headers.studying": "공부하는 분야:",
    "headers.interests": "관심이 있는 분야:",
    "headers.belief": "가지고 있는 의견",
    "headers.hobbies": "개발 외에 하고 있는 것",
    "headers.contact": "연락 방법:",
    "headers.projects": "최근에 만든 프로젝트들",

    "body.about.0":
      "저는 컴퓨터와 함께 자라며, 컴퓨터로 무엇을 할 수 있는지에 대해 늘 흥미를 가져왔습니다. 프로그래밍에 대한 관심은 어린 시절 컴퓨터 게임을 디자인하고 제작하면서 시작되었고, 예술가와 교육자로서의 경력을 거치며 점점 더 깊어졌습니다. 프로그래밍하는 일은 제 열정이며, 기술과 창의성을 융합할 수 있는 개발자가 되고 싶습니다.",
    "body.about.1":
      "새로운 기술과 트렌드를 중요하게 생각하지만, 어떤 역할에서도 도움이 되는 핵심 역량—프로그래밍 기본기, 좋은 시스템 설계, 효과적인 커뮤니케이션—을 더 중시합니다.",

    "body.working.0": "개인 웹사이트/포트폴리오 만들기",
    "body.working.1": "컴퓨터공학 석사과정 공부하기",
    "body.working.2": "대학생에게 데이터 기술 가르치기",
    "body.working.3": "특수교육 연구자들과 협력으로 앱 개발하기",

    "body.studying.0": "클라우드 엔지니어링",
    "body.studying.1": "소프트웨어 디자인",
    "body.studying.2": "백엔드 개발",

    "body.interests.0": "머신러닝 및 인공지능",
    "body.interests.1": "데이터베이스 구조",
    "body.interests.2": "컴퓨터 시스템",
    "body.interests.3": "창의적인 코딩",
    "body.interests.4": "그래픽스",

    "body.belief":
      "기술은 누구에게나 도움이 되어야 합니다. 저는 세상을 더 포용적으로 만드는 소프트웨어를 개발하고 싶습니다. 개발자는 신뢰할 수 있고 윤리적인 소프트웨어를 만들 의무가 있다고 믿습니다.",

    "body.hobbies.text":
      "아름다운 아내와 두 마리의 엉뚱한 고양이들과 함께 시간을 보내지 않을 때, 다양한 취미 활동으로 시간을 보냅니다.",
    "body.hobbies.list.0": "독서",
    "body.hobbies.list.1": "그림 그리기",
    "body.hobbies.list.2": "DIY",
    "body.hobbies.list.3": "게임",
    "body.hobbies.list.4": "운동",

    "body.contact": "이력서 다운로드 (pdf)",
  },
};
